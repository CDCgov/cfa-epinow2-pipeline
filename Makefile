...
deps:
	$(CNTR_MGMT) build -t $(REGISTRY)$(IMAGE_NAME)-dependencies:$(TAG) -f Dockerfile-dependencies

pull:
	az acr login --name 'cfaprdbatchcr'
	$(CNTR_MGMT) pull $(REGISTRY)$(IMAGE_NAME)-dependencies:$(TAG)
	$(CNTR_MGMT) pull $(REGISTRY)$(IMAGE_NAME):$(TAG)

build:
	$(CNTR_MGMT) build -t $(REGISTRY)$(IMAGE_NAME):$(TAG) \
		--build-arg TAG=$(TAG) -f Dockerfile .

tag:
	$(CNTR_MGMT) tag $(IMAGE_NAME):$(TAG) $(REGISTRY)$(IMAGE_NAME):$(TAG)
...
run-batch: config
	...
	sleep 15
	$(CNTR_MGMT) build -f Dockerfile-batch -t batch . --no-cache
	$(CNTR_MGMT) run --rm  \
	--env-file .env \
	-it \
...
run:
	$(CNTR_MGMT) run --mount type=bind,source=$(PWD),target=/mnt -it \
	--env-file .env \
...
up:
	$(CNTR_MGMT) run --mount type=bind,source=$(PWD),target=/cfa-epinow2-pipeline -it \
	--env-file .env \
...
push:
	$(CNTR_MGMT) push $(REGISTRY)$(IMAGE_NAME):$(TAG)
