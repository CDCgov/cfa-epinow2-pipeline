...
1. The following command will test your setup for using the `CFAEpiNow2Pipeline` package. This will run the pipeline for a single state and disease locally (using the computing power of your VAP account). This should take ~2 minutes. 
 `make run CONFIG=test/test.json`
2. The following command will test that your Azure resources are set up properly to send a task to an existing Azure Pool. This will run the pipefile for a single state (NY) on Azure Batch. You can monitor the state of the nodes and task status in Azure Batch Explorer. This should take ~15 minutes.
 `make test-batch`
